...
function RenderCal() {
  ...
  vCalHeader =
    '<table border=1 cellpadding=1 cellspacing=1 width=\'100%\' align="center" valign="top">\n';
  //Month Selector
  vCalHeader +=
  ...
  for (i = 0; i < 12; i++) {
    if (i == Cal.Month) SelectStr = "Selected";
    else SelectStr = "";
  ...
  if (ShowMonthYear)
    vCalHeader +=
      "<tr><td colspan='7'><font face='Verdana' size='2' align='center' color='" +
      MonthYearColor +
      "'><b>" +
  ...
  if (Cal.ShowTime) {
    ...
    vCalTime += " : ";
    vCalTime +=
      "<input type='text' name='minute' maxlength=2 size=1 style=\"WIDTH: 22px\" value=" +
      Cal.Minutes +
      ' onchange="javascript:winMain.Cal.SetMinute(this.value)">';
    vCalTime += " : ";
    vCalTime +=
      "<input type='text' name='second' maxlength=2 size=1 style=\"WIDTH: 22px\" value=" +
      Cal.Seconds +
    ...
    if (TimeMode == 12) {
      ...
      vCalTime +=
        '<select name="ampm" onchange="javascript:winMain.Cal.SetAmPm(this.options[this.selectedIndex].value);">';
...
function GenCell(pValue, pHighLight, pColor) {
  ...
  if (pColor != null) vColor = 'bgcolor="' + pColor + '"';
  else vColor = "";
  ...
  if (Cal.ShowTime) {
    vTimeStr =
      "winMain.document.getElementById('" +
      Cal.Ctrl +
      "').value+=' '+" +
